<template>
  <div>
    <div v-if="$page.props.flash.success && show" class="mb-8 flex items-center justify-between bg-green-500 rounded">
    ...
    <div v-if="($page.props.flash.error || Object.keys($page.props.errors).length > 0) && show" class="mb-8 flex items-center justify-between bg-red-400 rounded max-w-3xl">
      <div class="flex items-center">
        ...
        <div v-else class="py-4 text-white text-sm font-medium">
          <span v-if="Object.keys($page.props.errors).length === 1">The form contains 1 error</span>
          <span v-else>The format contains {{ Object.keys($page.props.errors).length }} errors.</span>
        </div>
      </div>
      <button type="button" class="group mr-2 p-2" @click="show = false">
